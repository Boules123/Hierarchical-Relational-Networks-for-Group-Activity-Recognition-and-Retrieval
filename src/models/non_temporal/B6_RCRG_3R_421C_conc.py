...
class B6_RCRG_3R_421C_conc(nn.Module):
    ...
    def forward(self, x):
        ...
        x1 = torch.cat([c1, c2, c3, c4], dim=1) # (b, n, 512)
        
        left = self.relational_layer2(x1[:, :n//2, :]) # (b, 6, 256)
        right = self.relational_layer2(x1[:, n//2:, :]) # (b, 6, 256)
        x2 = torch.cat([left, right], dim=1) # (b, n, 256)
        
        x3 = self.relational_layer3(x2) # (b, n, 128)
        
        x = x3.view(b, -1) # (b, n*128)
        out = self.fc(x) # (b, 8)
        return out
...
#   Accuracy: 86.98%  |  F1: 0.8695
#               precision    recall  f1-score   support

#        r_set       0.82      0.86      0.84      1728
#      r_spike       0.82      0.92      0.87      1557
#       r-pass       0.90      0.78      0.83      1890
#   r_winpoint       0.94      0.88      0.91       783
#   l_winpoint       0.94      0.96      0.95       918
#       l-pass       0.92      0.83      0.87      2034
#      l-spike       0.85      0.93      0.89      1611
#        l_set       0.85      0.88      0.86      1512

#     accuracy                           0.87     12033
#    macro avg       0.88      0.88      0.88     12033
