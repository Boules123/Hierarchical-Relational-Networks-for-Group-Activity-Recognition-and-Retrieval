"""Training and validation utilities for group-level classification."""
...
from src.models.model_registery import get_model
...
def train_one_epoch(cfg, model, dataloader, optimizer, criterion, device, epoch, scaler, writer, logger):
    model.train()
    total_loss = 0.0
    total_correct = 0
    all_preds = []
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        if (batch_idx + 1) % 50 == 0 or (batch_idx + 1) == len(dataloader):
            ...
            logger.info(f"Epoch {epoch+1}/{cfg.training.num_epochs} | Batch {batch_idx+1}/{len(dataloader)} | Loss: {batch_loss:.4f} | Accuracy: {current_acc:.4f}")
...
def validate(cfg, model, dataloader, criterion, device, epoch, writer, logger):
    model.eval()
    total_loss = 0.0
    total_correct = 0
    all_preds = []
    ...
    logger.info(f"Validation - Epoch [{epoch+1}/{cfg.training.num_epochs}] | Loss: {epoch_loss:.4f} | Accuracy: {epoch_acc:.4f} | F1 Score: {epoch_f1:.4f}")
    
    return epoch_loss, epoch_acc, epoch_f1


def train(cfg, seq=False):
    ...
    exp_dir = Path(cfg.experiment.save_dir) / f"{cfg.experiment.exp_name}_{timestamp}"
    exp_dir.mkdir(parents=True, exist_ok=True)

    writer = SummaryWriter(log_dir=str(exp_dir / "tensorboard"))
    logger = setup_logging(exp_dir=exp_dir)

    logger.info(f"Device: {device} | GPUs: {num_gpus} | Seed: {cfg.training.seed} | Experiment: {cfg.experiment.exp_name} | Version: {cfg.experiment.version}")
    ...
    train_loader = DataLoader(train_dataset, batch_size=effective_batch, shuffle=True, num_workers=2, collate_fn=collate_fn)
    val_loader = DataLoader(val_dataset, batch_size=effective_batch, shuffle=False, num_workers=2, collate_fn=collate_fn)
    ...
    ckpt_person = torch.load("", weights_only=False)
    ...
    optimizer = model.configure_optimizers(learning_rate=cfg.training.learning_rate, weight_decay=cfg.training.weight_decay)
    ...
    if cfg.training.resume_path and Path(cfg.training.resume_path).exists():
        ...
        logger.info(f"Resumed from: {cfg.training.resume_path} (epoch {start_epoch})")

    if num_gpus > 1:
        model = nn.DataParallel(model)
        logger.info(f"Using nn.DataParallel on {num_gpus} GPUs")
    ...
    for epoch in range(start_epoch, cfg.training.num_epochs):
        ...
        train_loss, train_acc = train_one_epoch(
            cfg, model, train_loader, optimizer, criterion, device, epoch, scaler, writer, logger
        )
        val_loss, val_acc, f1 = validate(
            cfg, model, val_loader, criterion, device, epoch, writer, logger
        )
        scheduler.step()
